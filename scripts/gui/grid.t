...
local m = {}
local Line = require("mesh/line.t")

function m.addLineCircle(dest, rad)
	-- create a circle
	local circlepoints = {}
	local npts = 60
...
function m.addSegmentedLine(dest, v0, v1, nsteps)
...
function m.createLineGrid(options)
	options = options or {}
	...
	for ix = 0,nx do
		...
		local v0 = {x, y0, 0}
		local v1 = {x, y1, 0}
		npts = npts + m.addSegmentedLine(lines, v0, v1, 30)
	end
	...
	for ir = 1,nr do
		npts = npts + m.addLineCircle(lines, r0 + ir*dr)
	end

	local color = options.color or {0.7,0.7,0.7,1}
	local linewidth = options.linewidth or 0.005

	local theline = Line(npts, false) -- static line
	theline:setPoints(lines)
	theline:createDefaultMaterial(color, linewidth)
