...
void Interpreter::attachAddon(Addon* addon) {
    if(!running_) {
        ...
        core()->log(TRUSS_LOG_ERROR, "Cannot attach addon to running interpreter.");
        delete addon;
    }
}
...
void Interpreter::setDebug(int debugLevel) {
    if (running_) {
        core()->log(TRUSS_LOG_WARNING, "Warning: Changing debug level on a running interpreter has no effect!");
    }
...
void Interpreter::execute() {
    core()->log(TRUSS_LOG_ERROR, "Interpreter::execute not implemented!");
...
void Interpreter::threadEntry() {
    ...
    if (!terraState_) {
        core()->log(TRUSS_LOG_ERROR, "Error creating a new Lua state.");
        running_ = false;
        return;
    }
    ...
    if (!bootstrap) {
        core()->log(TRUSS_LOG_ERROR, "Error loading bootstrap script.");
        running_ = false;
        return;
    }
    ...
    if(res != 0) {
        core()->log(TRUSS_LOG_ERROR) << "Error bootstrapping interpreter: "
    ...
    if (!call("_coreInit", arg_.c_str())) {
        core()->log(TRUSS_LOG_ERROR) << "Error in coreInit, stopping interpreter [" << id_ << "]\n";
        running_ = false;
    }
    ...
    // Shutdown
    core()->log(TRUSS_LOG_INFO, "Shutting down.");
...
bool Interpreter::call(const char* funcname, const char* argstr) {
    ...
    if(res != 0) {
        core()->log(TRUSS_LOG_ERROR, lua_tostring(terraState_, -1));
    }
