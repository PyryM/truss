...
function m.polygon_data(pts)
	local indices = m.triangulate(pts)
	return {attributes = {position = pts}, indices = indices}
end

function m.polygon_geo(pts, gname)
  local gfx = require("gfx")
  local geoutils = require("geometry/geoutils.t")
  local polygon_data = m.polygon_data(pts)
  geoutils.compute_normals(polygon_data)
  return gfx.StaticGeometry(gname):from_data(polygon_data)
end
