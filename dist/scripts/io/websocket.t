...
function WebSocketConnection:connect(url)
	...
	local success = wsAddon.truss_wsclient_open(wsAddonPointer, tostring(url))
	self.open = success
	return success
end

function WebSocketConnection:disconnect(url)
	wsAddon.truss_wsclient_close(wsAddonPointer)
	self.open = false
end
...
function WebSocketConnection:update()
	...
	local nmessages = wsAddon.truss_wsclient_receive(wsAddonPointer)
	if nmessages < 0 then
	...
	if self.decodeJSON then
		for i = 1,nmessages do
			local rawstr = wsAddon.truss_wsclient_getmessage(wsAddonPointer, i - 1)
...
function WebSocketConnection:send(msgstr)
	if not self.open then return end
	wsAddon.truss_wsclient_send(wsAddonPointer, tostring(msgstr))
end

function WebSocketConnection:sendJSON(obj)
	if not self.open then return end
	local jdata = json:encode(obj)
	wsAddon.truss_wsclient_send(wsAddonPointer, jdata)
end
